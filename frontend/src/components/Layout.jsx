...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#080a0f' }}>
      <header className="border-b flex items-center justify-between px-6 py-3 sticky top-0 z-50"
        style={{ background: 'rgba(8,10,15,0.97)', borderColor: '#1c2333' }}>
        ...
        <nav className="flex items-center gap-1">
          {NAV.map(n => (
            <NavLink key={n.to} to={n.to} end={n.to === '/'}
              className={({ isActive }) =>
                `px-3 py-1.5 rounded text-sm mono transition-all ${isActive ? 'text-white' : 'hover:text-white'}`}
        ...
        <div className="flex items-center gap-4 mono text-xs" style={{ color: '#4a5568' }}>
          {isRunning && (
            <span className="dot-pulse" style={{ color: '#ffd740' }}>
              ● {activeJob?.progress || 0}%
            </span>
          )}
          <span>{agents.length} agents</span>
          <span className="flex items-center gap-1" style={{ color: wsConnected ? '#00e676' : '#4a5568' }}>
            <span className={`w-1.5 h-1.5 rounded-full ${wsConnected ? 'dot-pulse' : ''}`}
              style={{ background: wsConnected ? '#00e676' : '#4a5568' }} />
